...
for y in ['M', 'K']:
    print(y + ": ")
    z1g_df = zad1_df[(zad1_df['Plec'] == y)].groupby('Imie').agg({'Liczba': ['sum']})
    z1g_a = z1g_df.columns.values[0]
    print(z1g_df.sort_values(z1g_a).tail(1))
...
#c)
print(zad3_df.groupby('Sprzedawca').agg({'Utarg': ['count']}))
#d)
